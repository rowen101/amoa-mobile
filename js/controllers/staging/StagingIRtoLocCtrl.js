app.controller('StagingIRtoLocCtrl', ['$scope', 'OrderStagingService', 'LoginService', '$ionicLoading','$ionicPopup' ,'LoadingService', '$rootScope', '$state', '$cordovaBarcodeScanner','$ionicModal',
        ...
        $scope.Scanshow = function(){
            ...
            if($rootScope.scanstaging.length === 0){
                return false;
                        ...
                        var ans = _.findWhere($rootScope.scanstaging, { stage: "637483115557783800" });        
                        if (ans) {
                          return true;
                        } else {
                          return false;
          ...
          /*  $scope.scan = $rootScope.scan;
                    var data = _.findWhere($rootScope.scan, {
                        stage: "637483036831001002"
                      }).pages;
                    if(data){
                        return true;
                    }else{
             var ans = _.findWhere(data, { stage: "637483115557783800" });        
                      if (ans) {
                        return true;
                      } else {
                        return false;
        ...
        $scope.ScanLocation = function () {
            ...
            $cordovaBarcodeScanner.scan().then(function (data) {
                if (!data.cancelled) {
                    ...
                    if(d != data.text){
                        $scope.loc = data.text.toUpperCase();
                        $scope.stagingConfirmation.show();
                      //  $scope.TransfertoStaging($scope.ir,loc,$scope.wc,$scope.emaialalias,$scope.notes);
                    }else{
            ...
            $scope.TransfertoStaging = function (ir,loc,wc,ea,notes) {
                ...
                OrderStagingService.TransfertoStaging(ir,loc,wc,ea,notes)
                    .then(function (res) {
                       ...
                       // $scope.stagelistdata = res.data;
                       // console.log($scope.stagelistdata);
