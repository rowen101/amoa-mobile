//Stock Replenishment Confirmation Controller
app.controller('StockReplenCfrmCtrl', ['$scope', 'LoadingService', 'PutAwayConfirmationSRV', '$rootScope', '$ionicPopup','$cordovaBarcodeScanner', '$ionicModal', 'LoginService', '$cordovaToast', '$state', '$timeout',"$cordovaCamera","$ionicPopup",
function ($scope, LoadingService, PutAwayConfirmationSRV, $rootScope,$ionicPopup, $cordovaBarcodeScanner, $ionicModal, LoginService, $cordovaToast, $state, $timeout,$cordovaCamera,$ionicPopup) {
...
return false;
          ...
          var ans = _.findWhere($rootScope.scanparent, { stage: "637471707370148338" });        
          if (ans) {
            return true;
          } else {
            return false;
...
$scope.getPutAwayList = function(data){
    LoadingService.StartLoading();
    PutAwayConfirmationSRV.GetActiveItemMasterListReplenish()
    .then(function (res) {
        $scope.itemCodeList = res.data;
        $scope.showConfirm(data);          
    }, function (err) {
        LoadingService.PopAlert("Something went wrong","Error Message: "+err.data+ "Sorry we cant get the Stock Replenishment List of Warehouse '" + $scope.wc + "'. Please try again later<br><br>Code : " + err.status + "<br> Status : " + err.statusText);
    }).
    finally(function () {
...
$scope.ImageUpdate = function (item) {
    ...
    var confirmPopup = $ionicPopup.confirm({
        ...
        subTitle: 'Stock Replenishment Confirmation Batch Number : '+item.BatchNo,
      });
      confirmPopup.then(function(res) {
        if(res) {

            $scope.GetAutoReplenConfirmation(item);
...
$scope.GetAutoReplenConfirmation = function (item) {
    LoadingService.StartLoading();
    PutAwayConfirmationSRV.GetStockReplenishmentConfirmation(item.BatchNo,item.WarehouseCode,item.TransactionCode,item.CreatedBy)
        .then(function (res) {
