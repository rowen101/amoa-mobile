//PickList Controller
app.controller('PutAwayConfirmationCtrl', ['$scope', 'LoadingService', 'PutAwayConfirmationSRV', '$rootScope', '$ionicPopup','$cordovaBarcodeScanner', '$ionicModal', 'LoginService', '$cordovaToast', '$state', '$timeout',"$cordovaCamera","$ionicPopup",
...
$scope.getPutAwayList = function(data){
    LoadingService.StartLoading();
    PutAwayConfirmationSRV.GetPutAwayList()
    .then(function (res) {
        $scope.itemCodeList = res.data;
        $scope.showConfirm(data);          
    }, function (err) {
        LoadingService.PopAlert("Something went wrong", "Error Message: "+err.data+" Sorry we cant get the PutAway List of Warehouse '" + $scope.wc + "'. Please try again later<br><br>Code : " + err.status + "<br> Status : " + err.statusText);
    }).
    finally(function () {
...
$scope.ImageUpdate = function (item) {
    ...
    var confirmPopup = $ionicPopup.confirm({
        ...
        subTitle: 'Put Away Confirmation Batch Number : '+item.BatchNo,
      });
...
$scope.GetAutoPutAwayConfirmation = function (item) {
    LoadingService.StartLoading();
    PutAwayConfirmationSRV.GetAutoPutAwayConfirmation(item.BatchNo,item.WarehouseCode,item.TransactionCode,item.CreatedBy)
        .then(function (res) {
