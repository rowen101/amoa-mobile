...
app.controller('ViewRcvBatchListCtrl', ['$scope', 'LoginService', '$ionicModal' ,'LoadingService', 'ReceiveSrv', '$cordovaBarcodeScanner','$timeout', '$cordovaToast', 'StorageService','ionicDatePicker', 
function($scope, LoginService,$ionicModal, LoadingService, ReceiveSrv, $cordovaBarcodeScanner,$timeout, $cordovaToast, StorageService,ionicDatePicker) {
    ...
    $scope.init = function () {
        ...
        $scope.batchlist = [];
        $scope.batchpick = {};
        $scope.title = "List of Batch on "+ $scope.wc;
        $scope.searchParam = '';
        $scope.getViewBatch();
    ...
    $scope.getViewBatch = function(){
    LoadingService.StartLoading(); 
      ReceiveSrv.getViewBatch()
        .then(function (res) {
