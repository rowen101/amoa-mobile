app.controller('LocationPickPickingCtrl', ['$scope','LoginService', 'LocationPickingService', 'LoginService','$ionicPopup', '$ionicLoading', 'LoadingService', '$rootScope', '$state', '$cordovaBarcodeScanner', '$ionicModal','$cordovaToast',
    function ($scope,LoginService, LocationPickingService, LoginService,$ionicPopup, $ionicLoading, LoadingService, $rootScope, $state, $cordovaBarcodeScanner,$ionicModal, $cordovaToast ) {
       $scope.pickDetail = {};
        ...
        $scope.init = function () {
            $scope.pickId = LocationPickingService.picklistId;
            $scope.GeItemToPick();
...
$scope.ScanLocation = function (itm) {
    $cordovaBarcodeScanner.scan().then(function (data) {
        ...
        $scope.GeItemToPick = function () {
            ...
            LocationPickingService.GeItemToPick(LocationPickingService.picklistId, LoginService.userProfile.warehouseCode)
                .then(function (res) {
                    $scope.list = LocationPickingService.pickingList;
                     if ($scope.list.length === 0) {
                    $state.go('app.2017081411380794');
                }
                }
                , function (err) {
