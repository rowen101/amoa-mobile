app.controller('VpLocIRPickingCtrl', ['$scope', 'VoicePickSrv', 'LoginService', '$ionicLoading', 'LoadingService', '$rootScope', '$state', '$cordovaBarcodeScanner', '$ionicModal', '$cordovaToast',
    function ($scope, VoicePickSrv, LoginService, $ionicLoading, LoadingService, $rootScope, $state, $cordovaBarcodeScanner, $ionicModal, $cordovaToast) {
        ...
        $scope.init = function () {
            $scope.ir = VoicePickSrv.issuanceRequest;
            $scope.GeItemToPick();
        }
        ...
        $scope.GeItemToPick = function () {
            ...
            VoicePickSrv.GetIRToPick($scope.ir, LoginService.userProfile.warehouseCode)
                .then(function (res) {
                    $scope.list = VoicePickSrv.irToPick;
                    if ($scope.list.length === 0) {
                        $state.go('app.636480928580666167');
                    }
                }
                , function (err) {
